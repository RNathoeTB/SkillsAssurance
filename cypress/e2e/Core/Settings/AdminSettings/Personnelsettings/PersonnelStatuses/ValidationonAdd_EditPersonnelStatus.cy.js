describe('Settings > Personnel settings', () => {
 ...
 cy.log('can not automate this validation step')
    ...
    //  cy.log('4. Enter valid name. Move down to the Description text field and enter more than 5000 characters (5001 and more), click \'SAVE\'')
     ...
     cy.get('.k-tabstrip-items').contains('Personnel statuses').click()
     cy.get('.svx-button > .telerik-blazor').click()
     const longName5000Plus = 'A'.repeat(5000)
     cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').type('Status 1')
     ...
     // On hover the following validation message is shown in the tooltip: 'Description has a maximum of 5000 allowed characters.'
cy.log('BUG: 5k plus characters in description is saved')

     cy.log('5. Enter valid number of characters and click \'SAVE\' button')
     cy.get('.svx-button > .telerik-blazor').click()
     cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').type('Status 1')
     cy.get(':nth-child(2) > .svx-formfield-content > .input-group > .k-textbox').type('validnumberofcharacters')
     cy.wait(3000)
     cy.get('.svx-modal-buttons > :nth-child(2) > .telerik-blazor').click()
     ...
     // The created item is available on the list
     cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').type('Status 1')
