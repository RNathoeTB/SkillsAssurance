...
//Bijvoorbeeld: inloggen voordat elke test wordt uitgevoerd
  ...
  cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper').type('Testcase2385ART')
  cy.get('.k-grid-content').should('contain', 'Testcase2385ART').dblclick()
// Clear the existing text
  cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').clear()
// Type the new text 'New Name'
  cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').type('Testcase2385')
...
it.only('Edit Personnel Status', () => {
  ...
  cy.get(':nth-child(3) > .svx-settings-container-body > :nth-child(3) > a').contains('Personnel settings').click()
  cy.get('.k-tabstrip-items').contains('Personnel statuses').click()
  cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper').type('Testcase2385')
  cy.get('.k-grid-content').should('contain', 'Testcase2385').dblclick()
//'Personnel status' pop-up appears. It contains all of the prefilled data.
  ...
  cy.get(':nth-child(1) > .svx-formfield-content > .input-group > .k-textbox').clear().type('Testcase2385ART')
...
// The new Name is visible in the grid in place of the old Name.
  ...
  cy.get('.k-tabstrip-items').contains('Personnel statuses').click()
  cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper').type('Testcase2385ART')
  cy.get('.k-grid-content').contains('Testcase2385ART')

  cy.log('3. Open the same record again. Change the \'Description\' field to another valid text. Click \'Save\'.')
  //cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper').clear().type('Testcase2385ART')
  cy.get('.k-grid-content').should('contain', 'Testcase2385ART').dblclick()
