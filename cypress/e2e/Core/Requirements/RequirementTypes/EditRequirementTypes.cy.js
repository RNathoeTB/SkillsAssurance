...
cy.get('#tree-item-8 > .k-link').click()
     ...
     cy.get('.k-master-row > [data-col-index="3"]').contains('True')
...
cy.log('5. Navigate to Personnel > Employees and open Employee having \'Requirement having type 1\' as requirement. Observe the \'Certificates\' tab.')
...
cy.log('BUG: code name is not updated')
     // cy.get('.condition').contains('step4')
     // cy.get('.ng-matrix-cell').contains('step4')
     
cy.log('6. Navigate to Requirements > Requirement types and open \'Requirement type 1\'. Disable \'Impacting compliance\' and click \'SAVE\'.')
cy.get('#tree-item-8 > .k-link').click()
     cy.get('#tree-item-8_3 > .k-link').click()
     cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper > .k-textbox').type('Requirement type 1').wait(3000)
     cy.get('.k-master-row > [data-col-index="1"]').contains('Requirement type 1').dblclick()
     cy.get('.k-switch-track').click().wait(2000)
     cy.get('.svx-modal-buttons > :nth-child(2) > .telerik-blazor').wait(2000).click().wait(3000)
     // Change is saved successfully.
     // The corresponding grid record is updated with new value.
     cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper > .k-textbox').find('.k-input-inner').clear().type('Requirement type 1').wait(3000)
cy.log('BUG: IC is not updated. refresh first')
     // cy.get('.k-master-row > [data-col-index="3"]').contains('False')

cy.log('7. Repeat step 5 and observe \'Certificates\' tab.')
     cy.get('#tree-item-4 > .k-link').click()
     cy.get('#tree-item-4_0 > .k-link').click()
     cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper > .k-textbox').find('.k-input-inner').type('Ritchie Nathoe').wait(3000)
     cy.get('.k-master-row > [data-col-index="1"]').contains('Ritchie Nathoe').dblclick().wait(3000)
     cy.get(' .k-link > .svx-font-2').eq(3).click().wait(6000)
      // 'Requirement having type 1' is not visible (having default filters applied).
cy.log('BUG:  \'Requirement having type 1\' is visible')
     cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper > .k-textbox').type('Requirement having type 1').wait(3000)
     // cy.get('.k-grid-norecords > .k-table-td').contains('No items to display')
    
cy.log('8. Set the \'Requirements\' filter to \'Show all\'')
    cy.get(':nth-child(2) > .svx-formfield-content > .k-combobox > .k-input-inner').clear().type('Show All').wait(2000).type('{enter}').wait(3000)
    cy.get('[data-col-index="1"] > .k-filtercell > .k-filtercell-wrapper > .k-textbox').clear().type('Requirement having type 1').wait(3000)
    cy.get('[data-render-row-index="2"] > [data-col-index="1"]').contains('Requirement having type 1')
